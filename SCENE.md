...
| 메서드 | 용도 |
|--------|------|
| `LoadAsset<T>(address)` | 동기 로드 (TextAsset, Sprite 등) |
| `LoadAssetAsync<T>(address, callback)` | 비동기 로드 |
| `LoadGameObject(address)` | 동기 프리팹 인스턴스 생성 |
| `LoadGameObjectAsync(address, callback)` | 비동기 프리팹 인스턴스 생성 |

- 캐시: `_addressablePacket` (주소 → 에셋 매핑)
